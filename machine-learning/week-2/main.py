...
def nn_cost_function(Thetas, X, y):
    ...
    # +1 want de bias-node zit wel in de matrix
...
print(
    f"De kosten die gemoeid zijn met de huidige waarden van Theta1 en Theta2 zijn {cost}")
...
res = minimize(nn_cost_function, init_params, args=args, method='CG',
               callback=callbackF, jac=True, options={'maxiter': 30, 'disp': True})
# voor de bias-node die wel in de matrix zit maar niet geplot moet worden
size = hidden_layer_size * (input_layer_size+1)
